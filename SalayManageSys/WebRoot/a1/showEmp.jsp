	...
	<div height="100%" style="margin-top:10px;">
   	...
   	<div style="margin-bottom:10px;">
		<s:form action="findEmp.action" method="post" theme="simple" name="f">
			工号：<s:textfield name="emp.uempid" cssClass="input"/>&nbsp;&nbsp;
			员工名称：<s:textfield name="emp.utruename" cssClass="input"/>&nbsp;&nbsp;
 	   ...
 	   <form action="" name="chform"  method="post" >
		  <c:forEach items="${emps}" var="emp" varStatus="i">
			<tr bgcolor="#FFFFFF">
				<td>${i.index+1 } </td>
				<td><a href="emp!load.action?emp.id=${emp.uid}">${emp.uempid}</a></td>
				<td>${emp.utruename}</td>
				<td>${emp.age}</td>
				<td>${emp.sex}</td>
				<td>${emp.deptname}</td>
				<td>${emp.startdate}</td>
				<td>
					<a href="emp!load.action?emp.id=${emp.uid}" >
					<img src="skin/edt.gif"/>修改</a>
					<a href="emp!delete.action?emp.id=${emp.uid}">
